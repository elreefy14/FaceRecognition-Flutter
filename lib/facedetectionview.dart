...
class FaceRecognitionViewState extends State<FaceRecognitionView> {
  ...
  void onUserFaceIdentified(Position userPosition, String companyId, String phoneNumber) async {
    ...
    if (isValid) {
      ...
      if (companySnapshot.exists) {
        ...
        // Parse the start time from Firebase (assuming "HH:mm" format)
        ...
        // Save identification and late time in the user's attendance subcollection
        await FirebaseFirestore.instance
            .collection('users')
            .doc(phoneNumber) // The user's phone number is the document ID
            .collection('attendance')
            .doc(dayMonthYear) // The document is identified by the day_month_year
            .set({
          'name': 'user_id_here', // Replace with the actual user ID
          'companyId': companyId,
  ...
  Future<bool> onFaceDetected(faces) async {
    ...
    if (faces.isNotEmpty) {
      ...
      if (maxSimilarity > _identifyThreshold && maxLiveness > _livenessThreshold) {
        ...
        onUserFaceIdentified(position,'holla', '01154354391');
